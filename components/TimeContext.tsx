...
function bridgeFor(year: string, era: string): TimeBridge {
  ...
  if (/(бал|военн|телесн|шпицрут|мазур)/.test(period)) {
    return {
      today: { title: "У тебя есть права и помощь", text: "Школа, закон и взрослые вокруг должны защищать ребёнка. Жестокость можно назвать жестокостью и попросить о помощи." },
      then: { title: "Чин и приказ сильнее голоса", text: "В Российской империи телесные наказания были частью военного порядка. Мундир означал не только уважение, но и власть." },
      difference: { title: "Один мундир — два взгляда", text: "Вечером форма говорит о красоте и статусе. Утром та же форма становится знаком системы, которая причиняет боль." },
    };
  }
